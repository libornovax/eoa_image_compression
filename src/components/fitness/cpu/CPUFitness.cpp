#ifndef CPUFITNESS_CPP
#define CPUFITNESS_CPP
...
namespace CPUFitness {
    ...
    void computeFitness (const std::vector<std::shared_ptr<CH>> &chromozomes, bool write_channels)
    {
        for (const std::shared_ptr<CH> &ch: chromozomes)
        {
    ...
    template<typename CH>
    void computeFitness (const std::shared_ptr<CH> &ch, bool write_channels)
    {
